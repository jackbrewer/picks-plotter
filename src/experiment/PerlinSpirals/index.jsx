...
const PerlinSpirals = ({
  ...
  for (let i = 1; i <= pointsCount; i++) {
    // const rotationPoint = i % numPoints
    const positionOffset =
      ((maxRadius - minRadius) / pointsCount) * i + minRadius
    const noise1 = noise(xOff, yOff)
    const x = c.x + Math.cos(i * angleStep) * positionOffset // + noise(xOff) * 10
    const y = c.y + Math.sin(i * angleStep) * positionOffset // + noise(yOff) * 10

    points.push({ x: x + noise1, y: y + noise1 })
    xOff += 1
    yOff += 1
  }
